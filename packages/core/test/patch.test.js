...
function makeFixture({ backgroundType } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-sidebar-api-fixture-'));
  const manifest = {
    manifest_version: 3,
...
test('patches a classic (non-module) service worker extension', () => {
  ...
  const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), 'arc-sidebar-api-output-'));

  const { manifest } = patchExtension({ sourceDir, outputDir });

  assert.equal(manifest.background.service_worker, '_arc_polyfill/service-worker-entry.js');
  const entry = fs.readFileSync(path.join(outputDir, '_arc_polyfill/service-worker-entry.js'), 'utf8');
  assert.match(entry, /importScripts\('sw-shim\.js', '\.\.\/background\.js'\)/);

  assert.ok(fs.existsSync(path.join(outputDir, '_arc_polyfill/floating-panel.js')));
  assert.ok(fs.existsSync(path.join(outputDir, '_arc_polyfill/sw-shim.js')));

  assert.ok(
    manifest.content_scripts.some((cs) => cs.js.includes('_arc_polyfill/floating-panel.js'))
  );
